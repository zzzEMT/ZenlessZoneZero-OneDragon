...
颜色范围无法从静态模板确定时，按以下步骤验证：

1. 确保目标代理人在战斗画面中，状态区域与实际队伍位置一致。
2. 运行采集脚本；脚本会从 `AgentEnum` 的 `state_list` 自动读取状态定义，并匹配已配置的状态模板区域和掩码。

   ```shell
   uv run python skills/agent-definition/collect_agent_state_hsv.py --agent <agent-id>
   ```

3. 脚本默认每 0.02 秒截图一次，采集 1 秒后输出每个状态的 H、S、V 连续区间；跳过坐标无效的模板区域。
4. 从输出中区分资源颜色主区间和灰色背景、UI 边缘产生的零散区间。`hsv_color` 是中心值，`hsv_color_diff` 是三通道容差，实际范围为“中心值 ± 容差”。
5. 以 H 作为主要颜色判据，按资源色相主区间设置较窄范围；S、V 应留出更宽的容差以覆盖抗锯齿、亮暗变化和特效。V 下限仍须排除暗色背景。
6. 设定范围后，分别在资源可用、资源消耗中和资源不可用的画面重新运行或实测检测结果；只在可用状态命中才可采用。
